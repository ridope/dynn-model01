# Ablation study model 02
# Activation function
# All activation functions changed to PReLu instead of LeakyRelu
...
class RSABlock(nn.Module):
    ...
    def forward(self, x, offset):
        if self.in_channel != self.out_channel:
            x = self.conv0(x)
        fea = self.prelu(self.dcnpack([x, offset]))
...
class OffsetBlock(nn.Module):
    ...
    def __init__(self, input_channel=32, offset_channel=32, last_offset=False):
        ...
        self.prelu = nn.PReLU(offset_channel)
        self.initialize_weights()

    def forward(self, x, last_offset=None):
        offset = self.prelu(self.offset_conv1(x))
        if last_offset is not None:
            last_offset = F.interpolate(last_offset, scale_factor=2, mode='bilinear', align_corners=False)
            offset = self.prelu(self.offset_conv2(torch.cat([offset, last_offset * 2], dim=1)))
        offset = self.prelu(self.offset_conv3(offset))
        return offset
...
class ContextBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        conv1 = self.prelu(self.conv1(x_reduce))
        conv2 = self.prelu(self.conv2(x_reduce))
        conv3 = self.prelu(self.conv3(x_reduce))
        conv4 = self.prelu(self.conv4(x_reduce))
...
class SADNET(nn.Module):
    ...
    def forward(self, x):
        conv1 = self.res1(x)
        pool1 = self.prelu(self.down1(conv1))
        conv2 = self.res2(pool1)
        pool2 = self.prelu(self.down2(conv2))
        conv3 = self.res3(pool2)
        pool3 = self.prelu(self.down3(conv3))
